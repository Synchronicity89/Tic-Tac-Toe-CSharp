...
for (int j = 0; j < 9; j++)
{
    ...
    Symbol[,] input = NeuralNetHelpers.GenerateRandomInput();
    ...
    double[] optimalOutput = engine.neuralNetwork.ScaledOptimalOutput(NeuralNetHelpers.InputToArray(input));

    // Feed the input forward through the network and get the predicted output
    double[] predictedOutput = engine.neuralNetwork.FeedForward(NeuralNetHelpers.InputToArray(input));
